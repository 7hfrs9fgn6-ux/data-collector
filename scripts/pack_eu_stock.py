...
欧洲数据打包模块（独立版）
用途：从采集数据重新打包为统一格式
使用：python scripts/pack_eu_stock.py
"""
...
def find_latest_raw() -> str:
    """找到最新的原始数据文件"""
    pattern = os.path.join(STAGING_DIR, "eu_stock_[0-9]*.json")
...
def main():
    parser = argparse.ArgumentParser(description='欧洲数据重新打包')
    parser.add_argument('--input', help='指定原始数据文件路径')
    args = parser.parse_args()

    logger.info("📦 欧洲数据重新打包")
    ...
    filepath = os.path.join(STAGING_DIR, f"eu_stock_package_{timestamp}.json")

    os.makedirs(STAGING_DIR, exist_ok=True)
    save_json(package, filepath)

    logger.info(f"✅ 打包完成: {filepath}")
    logger.info(f"   📊 指数数量: {package['content']['total']}")
